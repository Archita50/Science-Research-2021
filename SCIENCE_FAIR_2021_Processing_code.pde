...
String    heading = "Absorption Spectrun Graph";
String    credit  = "";
...
String[] lines = {
                 "Port:/dev/cu.usbserial-AK06G0OF",      //Serial Port
                 "Gain:1"};        //Gain (data amplificati
...
//---------- Show COM Port -----------------------------
  if (COMOPENED){
    ...
    text("Anemic:  " ,100, height-15);                  //Show Vertical Gain.
    text("Jaundice:  " ,230, height-15);    //Show Integration Period.
    text("A-Th: ",400, height-15);  //Show frames Per Second (FPS)
    text("B-Th:   SCD: ",480, height-15);  //Show frames Per Second (FPS)
...
//-------- If "settings.txt" doesn't exist, create it.-------------
  if (loadStrings("settings.txt")== null){
    ...
    comPortName= "/dev/cu.usbserial-AK06G0OF";                  //the substring after '='
